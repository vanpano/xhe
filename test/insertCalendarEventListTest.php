...
$app->command('test:test [ip] [port] [--restart] [--dynamical]', function($ip, $port, $restart, $dynamical) use (&$container, &$invoker) {
	...
	do {
		...
		//if (NULL === $loggedPrevious) {
			switch ($invoker->call('App\Command\GooglePlaygroundStep3', [ //$calendarId
				...
				//'body' => '{"accessRole": "freeBusyReader", "notificationSettings": {"notifications": [{"method": "email","type": "eventCreation"}]},"primary": ' . ($loggedPrevious === NULL ? 'true' : 'false') . ', "defaultReminders": [{"method": "email","minutes": 60},{"method": "popup","minutes": 60},{"method": "popup","minutes": 2880},{"method": "popup","minutes": 1440}]' . '}'
